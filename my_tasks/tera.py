...
class TeraClient(ipyparallel.Client):
    ssh_forward_child=None
    
    def __init__(self, username, profile_name="slurm"):
        json_file, self.json_filename = tempfile.mkstemp()
        os.close(json_file)

        scp_cmd = ("scp -o ConnectTimeout=10 " +
                   username + "@tera:.ipython/profile_{}/security/ipcontroller-client.json ".format(profile_name) +
                   self.json_filename)
        ...
        if result == 0:
            ...
            if result == 1:
                raise RuntimeError("Could not make ssh tunnel.")
            elif result == 2:
        ...
        super(TeraClient, self).__init__(self.json_filename)
